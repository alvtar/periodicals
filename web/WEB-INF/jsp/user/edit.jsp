...
<c:choose>
	<c:when test="${not empty user}">
        ...
        <c:set var="roleName" value="${user.role.name}"/>
		<c:set var="fullName" value="${user.fullName}"/>
		<c:set var="zipCode" value="${user.zipCode}"/>
		<c:set var="address" value="${user.address}"/>
		<c:set var="pageTitle" value="Пользователь ${user.fullName} ${user.zipCode} ${user.address}"/>
	</c:when>
	<c:otherwise>
		<c:set var="pageTitle" value="Новый пользователь"/>
	</c:otherwise>
</c:choose>
<u:html pageTitle="${pageTitle}" message="${message}" validator="validator-of-edit-user-form.js">
	<H2>${pageTitle}</H2>
